...
class UEditorController extends BaseController
{
    ...
    public function __construct($imageFieldName='')
    {
        if($imageFieldName)
        {
            ...
            $configKey = '';
            foreach($routesMap as $key => $val )
            {
                $configKey = $val;
                break;
            }
            $this->config = $configKey ? config('ueditor.'.$configKey) : '';
        }
    ...
    public function server(Request $request)
    {
        ...
        {
            $config = $this->config;
            $config['imageFieldName'] =  $this->imageFieldName;
        }else{
            $routesMap=config('ueditor.upload_routes_config_map');
            $route=$request->path();
        ...
        switch ($action) {
            ...
            default:
                $upConfig = array(
                    ...
                    'fieldName' =>  $this->imageFieldName ?  $this->imageFieldName : $config['fileFieldName'],
                );
                $result = with(new UploadFile($upConfig, $request))->upload();
